...
import { selectLan, validateForm, compareObjects, genreNames, generateDates, avgpopularity, } from "../helper";
...
function Movie() {
  const { isLoading, setIsLoading, posterImage, formatDate, data, } = useContext(MovieContext);
  const { id } = useParams();
  ...
  useEffect(() => {
    setIsLoading(true)
    fetchMovieById(id).then(res => { setIsLoading(false); setMovie(res.data[0]); setEntry(res.data[0]); setPopul(() => avgpopularity(res.data[0].original_title, (data || JSON.parse(localStorage.getItem('avgpopularity'))))) }).catch(e => navigate('/404'));
  }, [id]);
            ...
            <div className="row">
              <div className="col-md-6 py-2 px-5">
                <div className="bg-secondary p-3 rounded-2">
                  ...
                  {movie.popularity ? <h2>Popularity: {movie.popularity.toFixed(2)}</h2> : " "}
                  ...
                  {popul && <div>
                    <div className="row">
                      <div className="col-md-6 d-flex align-items-center">
                        <p className="mb-0 mr-2">This movie is popular</p>
